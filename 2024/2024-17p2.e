...
function String runme()
	...
	integer a, b, c, cmb, cmd, raw, clen, mat=0, tsum=-1
	List outt={}
	...
	clen=Length(cmds)
	for(count=1; count<=clen; count+=1)
		cmds[count]=Str.StringToInteger(cmds[count])
	end
	mat=1
	while mat <= clen
		a=tsum
		outt={}
		for(count=1; count<=clen; count+=2)
			cmd=cmds[count]
			raw=cmds[count+1]
