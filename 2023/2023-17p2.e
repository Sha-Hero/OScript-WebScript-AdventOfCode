...
function String runme()
	...
	while Length(Q) > 0
		...
		for entry in {{-1,0},{0,1},{1,0},{0,-1}}
			...
			isvalid = (new_indir<=mxr && (new_dir==dir || indir>=mnr || indir==-1))
			
			if rr>0 && rr<=Rs && cc>0 && cc<=Cs && isnt_reverse && isvalid
	...
	for (count=1; count<=Length(entry); count+=1)
		...
		review = entry[count]
		if (review[1]==Rs && review[2]==Cs && review[4] >= mnr)
			ans = Math.Min(ans, D.(review))
		end
	end
	echo ( "Total time: "+Str.String(Date.Tick()-Starter))
	//	echo("Distance is: "+Str.String(D.({Rs,Cs})))
